...
indep_vec_1 = Vec({'a'}, {'a':1})
indep_vec_2 = Vec({'a'}, {'a':2})
indep_vec_3 = Vec({'a'}, {'a':3})
indep_vec_4 = Vec({'a'}, {'a':-6})
...
def superset_basis(S, L):
    ...
    super_set = S + L
    slen = len(S)
    ans = copy.deepcopy(super_set)
    for i in range(len(L)):
        if is_superfluous(super_set,i+slen) == True:
            ans.remove(super_set[i+slen])
    return ans            
    pass
...
def exchange(S, A, z):
    ...
    for i in range(len(newSet)):
        if newSet[i] not in A:
            if is_superfluous(newSet,i) == True:
                return newSet[i]
    ...
    '''for elem in S:
        if elem not in A:
            tempSet = newSet
            tempSet.remove(elem) 
            ans = solve(coldict2mat(tempSet),elem)
            res = elem - coldict2mat(tempSet) * ans
            if res * res < pow(10,-5):
                return elem
    return S[0]
    '''
