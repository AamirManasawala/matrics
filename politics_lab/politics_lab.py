...
def create_voting_dict():
    ...
    for x in voting_data:
      ...
      ##if cur_line[1] != 'D':
        ##continue 
      voting_vec = list()
      for i,val in enumerate(cur_line):
        if i == 0 :
          last_name = val
        elif i !=1 and i!=2 :
          voting_vec.append(int(val))
      voting_dict[last_name] = voting_vec   
    return voting_dict 
...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    Example:
        ...
        -2
    """
    lista = voting_dict[sen_a]
    listb = voting_dict[sen_b]
    dot_product = 0
    for i in list(range(len(lista))):
      dot_product = dot_product + lista[i] * listb[i]     
    return dot_product
...
def most_similar(sen, voting_dict):
    ...
    """
    max_similar = ""
    max_dot_product = - 10**10  
    for k in voting_dict.keys():
    	if k!=sen:
          dot_product = policy_compare(sen,k,voting_dict)
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    for k in sen_set:
      if k != sen:
        total_dot_product += policy_compare(k,sen,voting_dict)
        total_can += 1            
    return float(total_dot_product/total_can)
...
def find_average_record(sen_set, voting_dict):
    ...
    sum_list = list()
    for x in sen_set:
      cur_vec = voting_dict[x]
      if i == 0:
        sum_list = cur_vec
        i = 1
      else:
        for j in range(len(cur_vec)):
          sum_list[j] = sum_list[j] + cur_vec[j]
          
    return [x/len(sen_set) for x in sum_list]
