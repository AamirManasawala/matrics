...
def lin_comb_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    result = Vec(M.D[0],dict())
    for key,col in mat2coldict(M).items():
        if key in v.f: 
           result = add(result,v[key] * col)
    return result
    pass
## Problem 12
def lin_comb_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    result = Vec(M.D[1],dict())
    for key,row in mat2rowdict(M).items():
        if key in v.f:
            result = add(result,v[key]*row)
    return result    
    pass
...
def dot_product_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    return Vec(M.D[0], {key:row*v for key,row in mat2rowdict(M).items()})
    pass
...
def dot_product_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    return Vec(M.D[1], {key:v*col for key,col in mat2coldict(M).items()})
    pass
...
def Mv_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    return coldict2mat({k:A* v for k,v in mat2coldict(B).items()})         
    pass
...
def vM_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    return rowdict2mat({k:v * B for k,v in mat2rowdict(A).items()})
    pass
...
def dot_prod_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    return Mat((A.D[0],B.D[1]), {(i,j):v1*v2 for i,v1 in mat2rowdict(A).items() for j,v2 in mat2coldict(B).items() })
...
solving_systems_m = Mat(({0, 1}, {0, 1}), {(0,0):(-1/5),(1,0):(2.0/5.0),(0,1):(4/5),(1,1):(-3/5)})
solving_systems_a = solving_systems_m
